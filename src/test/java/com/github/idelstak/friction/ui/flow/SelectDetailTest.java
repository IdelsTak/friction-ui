...
final class SelectDetailTest {
  ...
  @Test
  void showsReadyDetailStateAfterSelection(FxRobot robot) {
    WaitForAsyncUtils.waitForFxEvents();
    robot.clickOn("rétry flow unstable");
    WaitForAsyncUtils.waitForFxEvents();
    assertEquals("ready", robot.lookup("#detail-state").queryAs(Label.class).getText(), "detail state did not become ready after selection");
  }
}
